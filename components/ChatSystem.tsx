...
export const ChatSystem: React.FC<ChatSystemProps> = ({ currentUser }) => {
  ...
  const [usersPresence, setUsersPresence] = useState<
    Record<string, { text: string; isOnline: boolean }>
  >({});
  ...
  const getOtherParticipant = useCallback(
    (conv: Conversation) => {
      ...
      return (
        pros.find((p) => p.id === otherId) ||
        clients.find((c) => c.id === otherId)
      );
    },
    [currentUser.id]
  );
  ...
  const refresh = useCallback(() => {
    ...
    if (selectedConv) {
      const otherId =
        selectedConv.participants.find((p) => p !== currentUser.id) || "";
  ...
  // Fetch other user's presence when conversation is selected
  useEffect(() => {
    ...
    const fetchPresence = async () => {
      const lastSeen = await DB.getLastSeen(otherId);
      const presence = DB.formatPresence(lastSeen);
  ...
  // Fetch presence for all conversation participants (for list view)
  useEffect(() => {
    if (selectedConv || conversations.length === 0) return;

    const fetchAllPresence = async () => {
      const presenceMap: Record<string, { text: string; isOnline: boolean }> =
        {};

      for (const conv of conversations) {
        const otherId =
          conv.participants.find((p) => p !== currentUser.id) || "";
        if (otherId && !presenceMap[otherId]) {
          const lastSeen = await DB.getLastSeen(otherId);
          presenceMap[otherId] = DB.formatPresence(lastSeen);
        }
      }
    ...
    const interval = setInterval(fetchAllPresence, 60000);

    return () => clearInterval(interval);
  }, [conversations, selectedConv, currentUser.id]);
        ...
        <header className="px-4 pt-12 pb-4 flex items-center gap-4 bg-white border-b border-gray-100 shrink-0 shadow-sm relative z-10">
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            <div>
              ...
              >
                {otherUserPresence.text || "Cargando..."}
              </p>
            </div>
          </div>
              ...
              >
                ...
                <div className={`max-w-[75%] ${isMe ? "order-1" : ""}`}>
                  ...
                  >
                    ...
                    {isMe && (
                      <span
                        className={m.isRead ? "text-blue-500" : "text-gray-300"}
                      >
                        <Icons.Check />
                      </span>
                    )}
    ...
    <div className="flex-1 bg-gradient-to-b from-gray-50 to-white flex flex-col h-full animate-fade-in relative">
      ...
      <header className="px-6 pt-14 pb-4 shrink-0">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            onClick={() =>
              showToast("Reserva una clase para iniciar chat", "success")
            }
          >
            <Icons.Info />
          </button>
      ...
      <div className="flex-1 overflow-y-auto px-6 space-y-2 no-scrollbar pb-32">
        {conversations.map((conv, idx) => {
          ...
          const otherId =
            conv.participants.find((p) => p !== currentUser.id) || "";
          const presence = usersPresence[otherId];
          const hasUnread =
            conv.lastMessage && !conv.lastMessage.startsWith("✅");
        ...
        {conversations.length === 0 && (
          <div className="py-20 text-center flex flex-col items-center">
            ...
            <p className="text-xs text-gray-300 mt-1">
              Reserva una clase para chatear con profesionales
            </p>
          </div>
        )}
      </div>
