...
export const Home: React.FC<HomeProps> = ({
  ...
  const refresh = useCallback(() => {
    setPros([
      ...DB.getPros().filter((p) => p.status === "active" && p.planActive),
    ]);
    setCats([
      { name: "Todos", icon: "✨" },
      ...DB.getCategories()
  ...
  const filtered = useMemo(() => {
    ...
    return pros.filter((p) => {
      ...
      const matchCat =
        selectedCategory === "Todos" || p.areas.includes(selectedCategory);
  ...
  return (
    <div className="flex-1 flex flex-col bg-gradient-to-b from-slate-50 to-white overflow-hidden relative">
      ...
      <header className="px-6 pt-14 pb-4 shrink-0">
        ...
        >
          <div
            className={`absolute inset-0 bg-gradient-to-r from-blue-500/20 via-violet-500/20 to-purple-500/20 rounded-2xl blur-xl transition-opacity duration-300 ${
              ...
              >
                <div className="relative">
                  {p.image ? (
                    <img
                      src={p.image}
                      className="w-14 h-14 rounded-xl object-cover ring-2 ring-slate-100 group-hover:ring-blue-200 transition-all"
                      alt={p.name}
                    />
                  ) : (
                    <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-blue-500 to-violet-600 ring-2 ring-slate-100 group-hover:ring-blue-200 transition-all flex items-center justify-center text-white font-bold text-lg">
                      {p.name[0]}
                    </div>
                  )}
            ...
            <div className="flex-1 overflow-y-auto p-6 no-scrollbar space-y-3">
              ...
              {notifs.length === 0 && (
                <div className="py-16 text-center flex flex-col items-center">
                  ...
                  <p className="text-xs text-slate-300 mt-1">
                    Cuando tengas avisos aparecerán aquí
                  </p>
                </div>
              )}
            </div>
